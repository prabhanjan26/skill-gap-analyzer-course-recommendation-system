...
def _call_gemini(
    ...
    logger.info(
        "Calling Gemini model '%s' (temp=%s, timeout=%ss)",
    ...
    while True:
        try:
            ...
            logger.info("Gemini responded in %.1fs", time.perf_counter() - started)
            ...
            # Do NOT retry on timeout: a slow thinking model just times out again
            # and doubles the wait. Fail fast so the client isn't held for 2x.
...
def extract_skills(
    ...
    text = _call_gemini(system, user, temperature=0.1, max_output_tokens=8192, timeout=timeout)
    ...
    if parsed is None:
        ...
        text = _call_gemini(
            system, user + STRICTER_SUFFIX, temperature=0.0, max_output_tokens=8192, timeout=timeout
        )
        parsed = _parse_json_array(text)
...
def rerank_courses(
    ...
    timeout = get_settings().gemini_timeout_seconds
    # 8192 tokens: thinking models (gemini-2.5-*) spend part of this budget on
    # internal reasoning, so a small cap leaves no room for the JSON answer.
    text = _call_gemini(
        RERANK_SYSTEM_PROMPT, user, temperature=0.2, max_output_tokens=8192, timeout=timeout
    )
    parsed = _parse_json_array(text)

    if parsed is None:
        logger.warning(
            "Re-ranking JSON parse failed; retrying with stricter prompt. Raw (first 800 chars): %s",
            (text or "")[:800],
        )
        text = _call_gemini(
    ...
    if parsed is None:
        logger.error(
            "Re-ranking returned unparseable output; returning empty recommendations. Raw (first 800 chars): %s",
            (text or "")[:800],
        )
        return []
